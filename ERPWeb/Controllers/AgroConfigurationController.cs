...
namespace ERPWeb.Controllers
{
    public class AgroConfigurationController : BaseController
    {
        ...
        public ActionResult RawMaterialConverter(long FinishGoodProductId, long RawMaterialId, double FGRQty, long UnitId)
        {
            ...
            if (FinishGoodProductId == 1 && UnitId == 4)
            {
                if (RawMaterialId == 1)
                {
                    ConvertUnit = 1000 / FGRQty;
                    convertml = FGRQty * 1000;
                    cpa = 1250 * FGRQty / convertml;
                    RMQTY = cpa / ConvertUnit;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
                }
                else if (RawMaterialId == 2)
                {
                    ConvertUnit = 1000 / FGRQty;
                    convertml = FGRQty * 1000;
                    cpa = 5 * FGRQty / convertml;
                    RMQTY = cpa / ConvertUnit;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
                }
                else if (RawMaterialId == 3)
                {
                    ConvertUnit = 1000 / FGRQty;
                    convertml = FGRQty * 1000;
                    cpa = 2.5 * FGRQty / convertml;
                    RMQTY = cpa / ConvertUnit;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
                }
                else if (RawMaterialId == 4)
                {
                    ConvertUnit = 1000 / FGRQty;
                    convertml = FGRQty * 1000;
                    cpa = 5 * FGRQty / convertml;
                ...
                else if (RawMaterialId == 5)
                {
                    ConvertUnit = 1000 / FGRQty;
                    convertml = FGRQty * 1000;
                    cpa = 250 * FGRQty / convertml;
                    RMQTY = cpa / ConvertUnit;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
                }
            }

            else if (FinishGoodProductId == 1 && UnitId == 3)
            {
                if (RawMaterialId == 1)
                {
                    ConvertUnit = 1250f / 1000f;
                    RMQTY = ConvertUnit * FGRQty;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
                }
                else if (RawMaterialId == 2)
                {
                    ConvertUnit = 5f / 1000f;
                    RMQTY = ConvertUnit * FGRQty;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
                }
                else if (RawMaterialId == 3)
                {
                    ConvertUnit = 2.5f / 1000f;
                    RMQTY = ConvertUnit * FGRQty;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
                }
                else if (RawMaterialId == 4)
                ...
                {
                    ConvertUnit = 250f / 1000f;
                    RMQTY = ConvertUnit * FGRQty;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
                }

            }
            else if (FinishGoodProductId == 2 && UnitId == 1)
            {
                if (RawMaterialId == 6)
                {
                    convertml = 500f / 1000f;
                    RMQTY = convertml * FGRQty;
                    return Json(RMQTY, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult GetSendAndReceiveRequisitionReport(string RawMaterialRequisitionCode)
        {
            var data = _rawMaterialRequisitionInfoBusiness.GetSendAndReceiveReport(RawMaterialRequisitionCode);

            LocalReport localReport = new LocalReport();

            string reportPath = Server.MapPath("~/Reports/ERPRpt/Agriculture/rptSendAndReceiveRequisitionReport.rdlc");
            if (System.IO.File.Exists(reportPath))
            {
                localReport.ReportPath = reportPath;
            }

            ReportDataSource dataSource1 = new ReportDataSource("dsSendAndReceiveRequisitionReport", data);
            localReport.DataSources.Add(dataSource1);
        ...
        public ActionResult GetFinishGoodStockReport(string flag, long? productId, string fromDate, string toDate)
        {
            if (string.IsNullOrEmpty(flag))
            {
            ...
            else if (!string.IsNullOrEmpty(flag) && flag == Flag.View)
            {
                ...
                var dto = _finishGoodProductionInfoBusiness.GetFinishGoodStockReportList(productId ?? 0, fromDate, toDate);
                List<FinishGoodProductionInfoViewModel> viewModels = new List<FinishGoodProductionInfoViewModel>();
                AutoMapper.Mapper.Map(dto, viewModels);
                return PartialView("_GetFinishGoodStockReportList", viewModels);
        ...
        public ActionResult DateWiseCollection(string flag, long? zoneId, long? divisonId, long? regionId, long? areaId, long? stockiestId, long? territoryId, string fromDate, string toDate)
        {
            if (string.IsNullOrEmpty(flag))
            {
                ViewBag.ddlZoneName = _zoneSetup.GetAllZoneSetup(User.OrgId).Select(org => new SelectListItem { Text = org.ZoneName, Value = org.ZoneId.ToString() }).ToList();
        ...
        public ActionResult GetSalesReturnReportSave(string InvoiceNo)
        {
            ...
            string reportPath = Server.MapPath("~/Reports/ERPRpt/Agriculture/rptSalesReturnReportSave.rdlc");
            if (System.IO.File.Exists(reportPath))
            {
                localReport.ReportPath = reportPath;
            }
        ...
        {
            var StockiestName = _stockiestInfo.GetStockiestInfoById(id, User.OrgId).StockiestName;

            return Json(StockiestName, JsonRequestBehavior.AllowGet);

        }

        public ActionResult SoketistADD(long id)
        {
            var StockiestAddress = _stockiestInfo.GetStockiestInfoById(id, User.OrgId).StockiestAddress;

            return Json(StockiestAddress, JsonRequestBehavior.AllowGet);

        }
        public ActionResult SoketistNMBR(long id)
        {
            var StockiestPhoneNumber = _stockiestInfo.GetStockiestInfoById(id, User.OrgId).StockiestPhoneNumber;

            return Json(StockiestPhoneNumber, JsonRequestBehavior.AllowGet);

        }

        public ActionResult SoketistTerritoryId(long id)
        {
            var TerritoryId = _stockiestInfo.GetStockiestInfoById(id, User.OrgId).TerritoryId;
            var TerritoryName = _territorySetup.GetTerritoryNamebyId(TerritoryId, User.OrgId).TerritoryName;

            return Json(TerritoryName, JsonRequestBehavior.AllowGet);

        }
        public ActionResult SoketistTerritoryManager(long id)
        {
            var TerritoryId = _stockiestInfo.GetStockiestInfoById(id, User.OrgId).TerritoryId;
            var TerritoryManager = _territoryUserBusiness.GetTerritoryUserById(TerritoryId).UserId;

            var userName = _appUserBusiness.GetId(TerritoryManager, User.OrgId).UserName;

            return Json(userName, JsonRequestBehavior.AllowGet);

        }

        public ActionResult DealarLadgerList(long? id, string fromDate, string toDate)
        {
