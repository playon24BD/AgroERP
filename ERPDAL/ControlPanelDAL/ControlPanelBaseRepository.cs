...
namespace ERPDAL.ControlPanelDAL
{
    public class ControlPanelBaseRepository<T> : IBaseRepository<T> where T : class
    {
        ...
        public ControlPanelBaseRepository(IControlPanelUnitOfWork controlUnitOfWork)
        {
            if (controlUnitOfWork == null) throw new ArgumentNullException("DbContext is not assigned");
            this._controlUnitOfWork = controlUnitOfWork;
            dbSet = this._controlUnitOfWork.Db.Set<T>();
        }
        ...
        public bool Save()
        {
            ...
            return _controlUnitOfWork.Db.SaveChanges() > 0;
        }

        public async Task<bool> SaveAsync()
        {
            return await _controlUnitOfWork.Db.SaveChangesAsync() > 0;
        }
        ...
        {
            _controlUnitOfWork.Db.Entry(entity).State = System.Data.Entity.EntityState.Modified;
        }
        ...
        {
            using (var cmd = _controlUnitOfWork.Db.Database.Connection.CreateCommand())
            {
                cmd.CommandText = Sql;
